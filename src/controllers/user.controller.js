...
const updateUserCoverImager = asyncHandler(async (req, res) => {
    ...
    if (!coverImagerLocalPath) {
        throw new ApiError(400, "Cover Imager file is missing")
    }

    const coverImager = uploadOnCloudinary(coverImagerLocalPath)

    if (!coverImager.url) {
        throw new ApiError(400, "Error while uploading on Cover Imager")
    }

    const user = User.findByIdAndUpdate(
        req.user?._id,
        { $set: { coverImager: coverImager.url } },
        { new: true }
    ).select("-password")
